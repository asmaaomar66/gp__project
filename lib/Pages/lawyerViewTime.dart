     ...
     reservations.clear();
      await for(var reserveSnapshot in reservesStream ){
        for(var reserveDoc in reserveSnapshot.documents){
          var reservation ;
          var userSnapshot =await Firestore.instance.collection('users').
          document(reserveDoc['userId']).get();
           var timeSnapshot =await Firestore.instance.collection('Time').
           document(reserveDoc['timeId']).get();
           var userName=userSnapshot['fname']+" "+userSnapshot['lname'];
           reservation={"id":reserveDoc['reserveId'],
           "userName":userName,"date":timeSnapshot['date'],'userId':userSnapshot['id'],
           "time":timeSnapshot['time'],'case':reserveDoc['Case'],};
...
//-----------------------------------------------------------------------------------------------
Future _replayDialog(String id ,String userId) async {
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      onPressed: () {
                          Firestore.instance.collection('Reserve').document(id).updateData({ "pepar" : _replay.text });
                          DocumentReference documentRef = Firestore.instance.collection('notifiReplay').document();
                            documentRef.setData({
                         "userId":userId,
                         "state":false
                         });
                              ...
                              children: <Widget>[
                                RaisedButton(
                                 ...
                                 //_cancelDialog(doc['id'],doc['timeId']);
                                   _replayDialog(doc['id'],doc['userId']);
                                  },
                                  color: Color(0xffcb4154),
                                  textColor: Colors.white,
                                  child: Text("رد",
