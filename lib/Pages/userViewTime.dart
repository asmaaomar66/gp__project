...
var reservesStream=Firestore.instance.collection("Reserve").where("userId" , isEqualTo:  widget.currentUser.uid)
     ...
     reservations.clear();
      await for(var reserveSnapshot in reservesStream ){
        for(var reserveDoc in reserveSnapshot.documents){
          var reservation ;
          var lawyerSnapshot =await Firestore.instance.collection('users').document(reserveDoc['lawyerId']).get();
           var timeSnapshot =await Firestore.instance.collection('Time').document(reserveDoc['timeId']).get();
           var lawyerName=lawyerSnapshot['fname']+" "+lawyerSnapshot['lname'];
           reservation={"id":reserveDoc['reserveId'],"timeId":reserveDoc['timeId'],
           "lawyerName":lawyerName,"date":timeSnapshot['date'],
           "time":timeSnapshot['time'],'case':reserveDoc['Case'],'pepar':reserveDoc['pepar'],'lawyerId':reserveDoc['lawyerId'],};
  ...
  //-----------------------------------------------------------------------------------------------
     Future _dialogTime(String lawyerId,String timeId,String id) async {
    switch (await showDialog(
     ...
     stream: Firestore.instance.collection('Time').where("lawyerId" , isEqualTo: lawyerId )
              ...
              children: <Widget>[
                ...
                ,Column(
                   ...
                   crossAxisAlignment: CrossAxisAlignment.end,
                        ...
                        onTap: () {
                          Firestore.instance.collection('Time').document(timeId).updateData({ "state" : true });
                           Firestore.instance.collection('Reserve').document(id).updateData({ "timeId" : time['id'] });
                            Firestore.instance.collection('Time').document(time['id']).updateData({ "state" : false });
...
//------------------------------------------------------------------------------------------------------------------
  Future _cancelDialog(String id,String timeId) async {
    switch (await showDialog(
        context: context,
        builder: (BuildContext context) {
          return SimpleDialog(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      RaisedButton(
                        ...
                        onPressed: () {
               Firestore.instance.collection('Time').document(timeId).updateData({ "state" : true });
                                    Firestore.instance.collection('Reserve').document(id).delete();
                                    setState(() {
                                      getData();
                                    });
        ...
        var streamBuilder= StreamBuilder<List>(
         ...
         drawer: drawerprofile(currentUser: widget.currentUser,),
          ...
          child: Stack(children: <Widget>[
                              ...
                              children: <Widget>[
                                RaisedButton(
                                 ...
                                 //_cancelDialog(doc['id'],doc['timeId']);
                                   Firestore.instance.collection('Time').document(doc['timeId']).updateData({ "state" : true });
                                    Firestore.instance.collection('Reserve').document(doc['id']).delete();
                                    setState(() {
                                      getData();
                                    });
                                  },
                                ...
                                RaisedButton(
                                  shape: RoundedRectangleBorder(
                                      borderRadius: BorderRadius.circular(15.0),
                                      side:
                                          BorderSide(color: Color(0xff16bbbb))),
                                  onPressed: () {
                                    _dialogTime(doc['lawyerId'],doc['timeId'],doc['id']);
                                  },
           ...
           return Scaffold(body: Center(child:Text("انتظرقليلا..........")),);
         }
           });
